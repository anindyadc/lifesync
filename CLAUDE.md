...
- **WalletWatch** (`apps/walletwatch`) — expense tracker. Transactions carry amount (sign-normalized: `reimbursement` category forces positive, everything else negative), category, custom free-text tags, and a group/event label. Supports a "settle" flow (`relatedTxn`) that links a reimbursement entry back to the original expense and marks it `settled`. History view groups by month or by group/event tag, with progressive "Show More" disclosure. Categories are user-editable labels stored in the `settings/walletConfig` doc (`constants.js` seeds the defaults); tag/group colors are deterministic via `getTagColor` in `lib/utils.js`.
