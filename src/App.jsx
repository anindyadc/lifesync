...
export default function App() {
  ...
  const [activeApp, setActiveApp] = useState('dashboard'); // 'dashboard', 'taskflow', 'walletwatch'
      ...
      <nav className="w-full md:w-64 bg-slate-900 text-slate-300 flex-shrink-0 flex flex-col h-screen sticky top-0">
        <div className="p-6">
          <div className="flex items-center gap-3 text-white mb-8">
            ...
            <span className="font-bold text-lg tracking-tight">Project OS</span>
          </div>
          ...
          <div className="space-y-1">
            ...
            <button 
              onClick={() => setActiveApp('walletwatch')}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium ${
                activeApp === 'walletwatch' 
                  ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' 
                  : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
              <DollarSign size={18} /> WalletWatch
            </button>
          </div>
        </div>
        ...
        <header className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-800 capitalize tracking-tight">
              {activeApp === 'dashboard' ? 'Workspace Overview' : activeApp}
            </h1>
