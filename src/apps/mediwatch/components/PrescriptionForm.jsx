...
const PrescriptionForm = ({ onSubmit, onCancel, initialData = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSubmit({
        ...
        date,
        archiveOld,
        medicines: medicines.filter(m => m.name.trim() !== '') // Filter out empty medicines
      }, photoFile);
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="pt-4 border-t border-slate-100">
          <label className="block text-sm font-medium text-slate-700 mb-2">Prescription Photo</label>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            {photoPreview ? (
              <div className="relative w-full sm:w-48 h-48 rounded-xl overflow-hidden border-2 border-slate-200 bg-slate-50">
                <img src={photoPreview} alt="Prescription preview" className="w-full h-full object-cover" />
                <button
                  type="button"
                  onClick={() => {
                    setPhotoFile(null);
              ...
              <div className="w-full sm:w-48 h-48 flex flex-col items-center justify-center border-2 border-dashed border-indigo-200 bg-indigo-50/50 rounded-xl overflow-hidden">
            ...
            <div className="text-sm text-slate-500 flex-1">
              Take a photo of the physical prescription with your mobile camera or upload an existing image.
              {fileError && <p className="text-red-500 mt-2 font-medium animate-pulse">{fileError}</p>}
              <p className="text-[10px] mt-1 text-slate-400 uppercase tracking-wider font-bold">Max size: 2MB</p>
            </div>
          </div>
        </div>
