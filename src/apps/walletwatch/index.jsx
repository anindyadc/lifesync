...
import { Settings, Plus, X, Trash2, LayoutGrid, LayoutList, Loader2 } from 'lucide-react';
...
import { CategorySubtotals } from './components/DashboardStats';
 ...
 * Restores all features: Weekly Activity, Reimbursement Settlements, and Event Grouping.
 */
const WalletWatchApp = ({ user }) => {
  ...
  const [relatedTxn, setRelatedTxn] = useState(null); // Critical for Settle/Refund logic
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-20 space-y-4">
        ...
        <p className="text-slate-400 text-sm font-medium italic">Syncing your vault...</p>
      </div>
    );
  }
  ...
  /**
   * handleSave
   * Handles adding new transactions, editing existing ones, 
   * and processing settlements (refunds) for lent money.
   */
  const handleSave = async (formData) => {
    const col = collection(db, 'artifacts', APP_ID, 'users', user.uid, 'expenses');
    
    // Logic: If it's a settlement (refund), store the amount as negative
    const finalAmount = relatedTxn ? -Math.abs(Number(formData.amount)) : Number(formData.amount);

    const payload = {
      ...formData,
      amount: finalAmount,
      // Convert string date from form back to Firestore Timestamp
    ...
    try {
      if (editingId) {
        ...
        // If we are settling a "Lent" transaction, mark the original source item as settled
        if (relatedTxn) {
  ...
  /**
   * handleSettleRequest
   * Pre-fills the form to record a refund for a previously lent transaction.
   */
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 font-sans">
      <ConfirmModal 
        ...
        message="This action is permanent and will update your dashboard totals." 
        onConfirm={() => {
          // You can implement the deleteDoc call here using deleteId
          setDeleteId(null);
        }} 
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex gap-2 bg-slate-100 p-1 rounded-xl">
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${view === 'history' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
        ...
        <button 
          ...
          className="p-2.5 bg-slate-50 text-slate-600 rounded-xl border border-slate-100 hover:bg-slate-100 transition-all active:scale-95"
        >
          <Settings size={20}/>
        </button>
      </div>
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-sm shadow-2xl p-8 animate-in zoom-in-95">
            ...
            <div className="flex gap-2 mb-6">
              <input 
                ...
                className="flex-1 px-4 py-2 bg-slate-50 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-medium" 
              />
              <button 
                onClick={handleAddCategory} 
                className="p-3 bg-indigo-600 text-white rounded-2xl shadow-lg active:scale-90 transition-all"
              >
                <Plus size={20}/>
              </button>
            </div>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {categories.map(c => (
                <div key={c.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-2xl group border border-transparent hover:border-indigo-100 transition-all">
